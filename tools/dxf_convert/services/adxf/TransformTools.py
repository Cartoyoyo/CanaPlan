# -*- coding: utf-8 -*-
# Adapted from AnotherDXF2Shape/TransformTools.py
...
def Helmert4Points(p1, p2):
    ...
    def sumQS_xy(KooArray1, xS1, KooArray2, yS2):
        ...
        for i in range(len(KooArray1)):
            erg += (xS1 - KooArray1[i][0]) ** 2 + (yS2 - KooArray1[i][1]) ** 2
        return erg
    def sumP_x(KooArray1, xS1, KooArray2, xS2):
        erg = 0.0
        for i in range(len(KooArray1)):
            erg += (xS1 - KooArray1[i][0]) * (xS2 - KooArray2[i][0])
        return erg
    def sumP_y(KooArray1, yS1, KooArray2, yS2):
        erg = 0.0
        for i in range(len(KooArray1)):
            erg += (yS1 - KooArray1[i][1]) * (yS2 - KooArray2[i][1])
        return erg
    def sumP_xy(KooArray1, xS1, KooArray2, yS2):
        erg = 0.0
        for i in range(len(KooArray1)):
            erg += (KooArray1[i][0] - -xS1) * (KooArray2[i][1] - yS2)
        return erg
    def Max_xy(KooArray):
        maxX = KooArray[0][0]; maxY = KooArray[0][1]
        for i in range(len(KooArray)):
            if KooArray[i][0] > maxX: maxX = KooArray[i][0]
            if KooArray[i][1] > maxY: maxY = KooArray[i][1]
        return maxX, maxY
