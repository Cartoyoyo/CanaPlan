...
class TrenchSchemaWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # Define layers to draw with their relative visual weights
        weights = []
        ...
        def get_color(mat):
            ...
            if 'sable' in mat_lower: return QColor(245, 235, 175) # Jaune sable
            if '2/6' in mat_lower: return QColor(210, 210, 210) # Gris clair
            if '0/31.5' in mat_lower or 'tout-venant' in mat_lower: return QColor(175, 175, 175) # Gris moyen
            if 'gb' in mat_lower or 'bitume' in mat_lower: return QColor(100, 100, 100) # Gris foncé
            if 'gc' in mat_lower or 'ciment' in mat_lower: return QColor(180, 180, 185) # Gris ciment
            if 'enrobé' in mat_lower: return QColor(50, 50, 50) # Asphalte noir
            return QColor(220, 220, 220) # Défaut
        ...
        for layer_id, weight, label, ep_str, mat in weights:
            ...
            # Gestion de la couleur du texte (contraste)
            lum = color.red() * 0.299 + color.green() * 0.587 + color.blue() * 0.114
            text_color = Qt.black if lum > 128 else Qt.white
            
            # Dessin de la conduite dans la zone d'enrobage
            if layer_id == 'enr':
                # La conduite repose sur le lit de pose (fond de l'enrobage)
                diam = min(layer_h * 0.55, draw_w * 0.3)
                radius = diam / 2
                cx = margin_x + draw_w / 2
                cy = current_y + layer_h - radius # le bas du cercle touche le fond de la zone
                
                painter.setBrush(QBrush(QColor(255, 255, 255))) # Conduite blanche
                painter.setPen(QPen(Qt.black, 2))
                painter.drawEllipse(int(cx - radius), int(cy - radius), int(radius * 2), int(radius * 2))
                
                # Ligne pointillée pour la génératrice supérieure
                dim_x = cx
                ...
                # Ligne de cote pour l'épaisseur (au-dessus génératrice)
                ...
                # Petits traits d'extrémité (ticks)
            ...
            # Rétablissement de la police pour les labels
            painter.setPen(text_color)
            font = QFont(original_font)
            
            # Nom de la couche (gauche)
            font.setBold(True)
            ...
            # Epaisseur (colonne du milieu, alignée verticalement)
            if ep_str:
                if layer_id == 'enr':
                    # On centre le texte dans la zone au-dessus de la génératrice supérieure
            ...
            # Matériau (droite)
            if layer_id == 'enr':
                # On centre le texte du matériau à côté de la conduite (sous la génératrice) pour éviter le trait
...
class CubatureSchemaWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # Echelle visuelle: on map de 0.4m à 2.0m vers 20% à 80% de la largeur du widget
        ...
        # Dessiner le sol (terre)
        painter.setPen(Qt.NoPen)
        painter.setBrush(QBrush(QColor(185, 155, 115))) # Couleur terre
        
        # Terrain gauche et droite
        ...
        # Ligne de surface (TN) et bords de tranchée
...
class NetworkSchemaWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        margin_y = 10
        draw_w = w - 2 * margin_x
        draw_h = h - margin_y - 35 # Espace pour le texte en bas
        
        def get_pipe_color(mat):
            mat_low = mat.lower() if mat else ""
            if 'pvc' in mat_low: return QColor(225, 120, 50) # Orange PVC
            if 'fonte' in mat_low: return QColor(60, 60, 60) # Gris très foncé
            if 'beton' in mat_low or 'béton' in mat_low: return QColor(190, 190, 190)
            if 'pehd' in mat_low: return QColor(30, 30, 30) # Noir
            if 'acier' in mat_low: return QColor(110, 140, 160) # Bleu acier
            if 'gres' in mat_low or 'grès' in mat_low: return QColor(160, 70, 40) # Terre cuite
            return QColor(150, 150, 150) # Défaut
            
        diam_c = self.data.get('diam_cond', 200)
        diam_b = self.data.get('diam_branch', 160)
        
        # On met à l'échelle pour que le plus gros tuyau occupe 80% de la hauteur dispo
        ...
        # Ligne de sol commune pour l'esthétique
...
class ConfigDialog(QDialog):
    def __init__(self, iface, parent=None):
        ...
        self._def_widgets = {}
        self._cub_widgets = {}
        self._remblai_widgets = {}  # spinboxes
        self._remblai_mat = {}      # combos matériaux
        self._remblai_cb = {}       # checkboxes
        self._network_schemas = {}  # widgets de dessin réseaux
        self._build_ui()
        self._load_settings()
    ...
    def _build_ui(self):
        ...
        # Ajout du schéma de largeur dynamique
        ...
        # Schéma coupe tranchée (dynamique via QPainter)
        ...
        # Connecter tous les widgets remblai pour mise à jour du schéma
        ...
        # Initialiser les schemas
        self._refresh_network_schemas()

    def _refresh_network_schemas(self):
        if not hasattr(self, '_network_schemas'): return
        
        for reseau in ("EU", "EP"):
            if reseau not in self._network_schemas: continue
