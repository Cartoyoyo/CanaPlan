...
_MISSING_CONVERTER_MSG = (
    "Aucun convertisseur DWG → DXF n'a ete trouve.\n\n"
    "Pour lire des fichiers DWG, installez l'un des logiciels suivants :\n\n"
    "  ODA File Converter (recommande sur Windows) :\n"
    "    https://www.opendesign.com/guestfiles/oda_file_converter\n"
    "    Telegcharger la version gratuite, installer, puis redemarrer QGIS.\n\n"
    "  LibreDWG (open source, necessite une compilation sur Windows) :\n"
    "    https://www.gnu.org/software/libredwg/\n\n"
    "Alternative : exporter le DWG en DXF depuis votre logiciel CAO."
)


def get_converter_help() -> str:
    """Retourne un message d'aide a afficher a l'utilisateur."""
    return _MISSING_CONVERTER_MSG


def check_dwg_converter_available() -> bool:
    """Retourne True si au moins un convertisseur DWG est disponible."""
    return bool(find_oda() or find_libredwg())
...
def dwg_to_temp_dxf_auto(dwg_path: str, prefer: str = "auto", dxf_version: str = "ACAD2013") -> str:
    ...
    try:
        ...
        oda_ok = find_oda()
        dwg_ok = find_libredwg()
        if not oda_ok and not dwg_ok:
            raise RuntimeError(_MISSING_CONVERTER_MSG)
        try:
            return convert_with_oda(dwg_path, tmpdir, dxf_version=dxf_version)
        except Exception as e_oda:
            if not dwg_ok:
                raise RuntimeError(
                    f"ODA File Converter a echoue : {e_oda}\n\n{_MISSING_CONVERTER_MSG}"
                )
            return convert_with_libredwg(dwg_path, tmpdir)
    except Exception:
