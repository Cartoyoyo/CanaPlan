...
class RenommerTool(QgsMapTool):
    ...
    def _build_graph(self):
        r_pts = [
            ...
            for feat in self.couches['regard'].getFeatures()
        ...
        def snap(pt):
            ...
            return best_id if best_d <= self._SNAP_TOL_M else None

        graph = {}
        for feat in self.couches['conduite'].getFeatures():
            g = feat.geometry()
            if g.isEmpty():
                continue
            line = g.asPolyline()
    ...
    def _bfs(self, graph, start_id, end_id):
    ...
    def _rename_path(self, start, end):
        ...
        r_ids, c_feats = self._bfs(graph, start.id(), end.id())
